...
- The right-sidebar panel is `logseq.Experiments.registerSidebarRenderer` (`src/main.ts`), with the displayed rows built by the pure `src/panel.ts`. That API only exists in `@logseq/libs` 0.3.x — npm's `latest` tag is still 0.0.17, so the DB-era SDK comes from an explicit version. Registration crosses into the host through `window.top` and the panel is rendered by the **host's** React (`logseq.Experiments.React`, typed `unknown`), so it depends on the same root `"effect": true` as the live-editor path. The authoritative reference for this surface is `libs/guides/experiments_api_guide.md` in the `logseq/logseq` repo; plugins-doc.logseq.com does not cover it.
