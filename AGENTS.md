...
- `src/time-prefix.ts` holds the pure prefix helpers and is what `src/time-prefix.test.ts` covers; `src/settings-panel.ts` holds the popover's field descriptors, placement and value mapping (covered by `src/settings-panel.test.ts`) next to its DOM builder, which Vitest runs no DOM for. `src/main.ts` is Logseq runtime wiring (settings schema, editor and DB event handlers, panel lifecycle) and has no tests. Put new logic in a pure helper so it is testable, and keep `main.ts` thin.
...
- The UI the plugin owns is the toolbar button plus the settings popover it toggles (`registerToolbarItem`, `toggleSettingsPanel` in `src/main.ts`). `logseq.App.registerUIItem('toolbar', { key, template })` renders its template in the **host**, so the click handler must be a named method registered through `logseq.provideModel` and referenced as `data-on-click` — a closure cannot cross that boundary. Logseq parks plugin toolbar items in the toolbar plugins popover until the user pins one. A right-sidebar panel via `logseq.Experiments.registerSidebarRenderer` was shipped in #10 and removed again in favor of this; `@logseq/libs` stays on the explicit 0.3.x version it needed, since npm's `latest` tag is still 0.0.17.
- The popover lives in the host document, not the plugin iframe, which is what lets it inherit Logseq's theme variables (`--ls-primary-background-color` and friends) and follow light/dark for free. `logseq.provideUI({ path: 'body', … })` contributes only the empty container — the host mounts it asynchronously over the plugin bridge, so `src/main.ts` polls for it — and `src/settings-panel.ts` builds and wires the controls with DOM APIs from the plugin side, which the same-origin `"effect": true` iframe allows. Two consequences: event handlers work normally (no `data-on-click` dance beyond the toolbar item), but host-document nodes come from the parent realm, so `instanceof Node`/`Element` is false there and membership checks have to be structural. Styles ship through `logseq.provideStyle`. Without a reachable host document the popover cannot mount, so the click falls back to `logseq.showSettingsUI()`.
...
- Logseq's cycle-todo shortcut (`cmd`/`ctrl` + `Enter`) is a *second* way into that same shape, and a harder one: measured live, it converts the block being edited into a task node **in place** — same uuid, same textarea, text untouched — and its `Task` tag lands ~40 ms after the keydown, i.e. after the `setTimeout(0)` context refresh `onEditorStructureKeydown` schedules. So there is no in-band signal like a slash command's `/`, and re-reading the block from the keydown is still too early; the answer has to be re-checked when the prefix is about to be inserted (`isTaskCycleShortcut` feeding `commandedEditors`).
