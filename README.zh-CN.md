...
插件在顶栏的插件区域（Logseq 自带设置按钮旁）添加一个时钟按钮。点击即在按钮下方弹出一个紧凑的设置面板，配色取自 Logseq 自身的主题变量，可在其中编辑格式与两个排除列表。面板会显示下一个 block 将得到的前缀，边改边生效，按 `Esc`、点击面板外部或点右上角关闭按钮即收起。`Full settings / 完整设置` 打开 Logseq 内置的设置面板，编辑的是同一批选项。

Logseq 把插件按钮收在顶栏的插件弹出面板里，打开该面板并把 `Journal Time Prefix` 固定（pin），按钮就会常驻顶栏。
...
3. 需要改前缀样式或排除某些内容时，点顶栏时钟按钮在弹出面板里改，或在 `Plugins` → `Journal Time Prefix` → `Settings` 里调整上面几项设置。
